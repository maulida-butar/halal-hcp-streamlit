...
📊 Metodologi dan FormulaSkor risiko total awal pada tiap tahapan dihitung menggunakan metode rata-rata berbobot konvensional:$$\text{Risk Score} = (w_B \times B) + (w_K \times K) + (w_A \times A) + (w_D \times D) + (w_S \times S)$$Di mana $w$ merupakan bobot kepentingan relatif masing-masing indikator. Logika model kemudian mengaplikasikan fungsi pembatas (constraint):$$\text{Status} = \begin{cases} \text{Critical Point}, & \text{jika } \exists \\, X \in \\{B, K, A, D, S\\} > T_v \\\\ \text{Classify}(X), & \text{jika } \forall \\, X \le T_v \end{cases}$$Di mana $T_v$ merupakan Veto Threshold (ambang batas kritis dinamis) yang diatur interaktif oleh pakar/pendamping halal.
